...
- автоматический перенос временной распаковки TG WS Proxy на другой локальный диск, если системный диск переполнен;
...
Для распаковки TG WS Proxy требуется не менее 256 МБ свободного места. Если системная временная папка переполнена, менеджер автоматически использует собственную папку `FlowsealManagerTemp\TgWsProxy` на доступном локальном диске и не меняет общесистемные переменные Windows.

Установщик также предлагает включить штатный режим автонастройки TCP `normal` командой Windows `netsh int tcp set global autotuninglevel=normal`. Флажок виден на странице задач установки и при необходимости снимается; диагностическая команда `netsh int tcp show global` ничего не изменяет и в установщик не добавляется.
