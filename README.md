...
- автоматическое обновление самого Flowseal Manager из этого GitHub-репозитория с проверкой размера, SHA-256 и версии установщика;
...
Скачайте единственный файл `FlowsealManager-Setup.exe` из [последнего релиза](https://github.com/DmitryKalibrov/flowseal-manager/releases/latest). Установщик сам выберет версию для Windows x64 или ARM64, создаст ярлык в меню «Пуск» и запустит менеджер. Отдельные ZIP, JSON и файл хешей пользователю не нужны.
...
Текущая локальная сборка не подписана коммерческим code-signing сертификатом, поэтому SmartScreen может показать предупреждение «Неизвестный издатель». 
...
Каждый новый стабильный GitHub Release содержит один файл: `FlowsealManager-Setup.exe`. Внутри находятся обе сборки; установщик сам выбирает x64 или ARM64.

Flowseal Manager проверяет `releases/latest`, сверяет точное имя, размер, SHA-256 из GitHub Release API и версию EXE. Только после всех проверок он запускает тихое обновление. Настройки, скачанные компоненты Flowseal, пользовательские списки и системный `hosts` находятся вне каталога приложения и не заменяются.

Переход с `v1.1.0` на единый установщик `v1.2.0` выполняется вручную один раз. Следующие стабильные релизы снова устанавливаются автоматически.

## Выпуск новой версии

1. Измените `ReleaseVersion` и `BuildVersion` в `Directory.Build.props`.
2. Подготовьте русское описание `RELEASE-X.Y.Z.md`.
3. Установите Inno Setup 6 и соберите единый установщик командой `./scripts/package-release.ps1`.
4. Проверьте содержимое каталога `release` и поставьте тег, совпадающий с `ReleaseVersion`, например `v1.2.0`.
5. Отправьте тег на GitHub. Workflow проверит тесты и номер тега, соберёт единый `FlowsealManager-Setup.exe` для x64/ARM64 и опубликует GitHub Release с подготовленным описанием.
